...
GstRTSPFilterResult ClientFilterFunc (GstRTSPServer *server,
                                  ...
                                  gpointer user_data)
{
  ...
  //GST_RTSP_FILTER_REF,
	  ...
	  //priv->server_ip;
	   printf("remove client path %s  %p\n",  gst_rtsp_client_get_path(client), client);
	   // GST_INFO ("reusing cached media  for path %s",  priv->path);
	 //return GST_RTSP_FILTER_REMOVE;
	//return GST_RTSP_FILTER_KEEP;
}

static gboolean
remove_client (GstRTSPServer * server)
{
  gst_rtsp_server_client_filter (server,ClientFilterFunc, NULL);
  return 1;
}
...
main (int argc, gchar * argv[])
{
  ...
  GstRTSPMediaFactoryURI *factory1;
  ...
  gst_rtsp_server_set_backlog(server, 100);
  ...
  factory1 = gst_rtsp_media_factory_uri_new ();
...
gst_rtsp_media_factory_set_shared(factory1,1);
 ...
 gst_rtsp_media_factory_uri_set_uri (factory1, "udp://0.0.0.0:60002");
  g_free (uri);
  ...
  gst_rtsp_mount_points_add_factory (mounts, "/test1",
      GST_RTSP_MEDIA_FACTORY (factory1));
  ...
  g_timeout_add_seconds (2, (GSourceFunc) timeout, server);
    g_timeout_add_seconds (10, (GSourceFunc) remove_client, server);
