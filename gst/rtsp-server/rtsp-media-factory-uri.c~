    ...
    GstElement * element)
{
  ...
  GST_DEBUG ("found pad %s:%s of caps %" GST_PTR_FORMAT,
   ...
   GstStructure *structure11 = gst_caps_get_structure (caps, 0);
  gchar *na = gst_structure_get_name(structure11);
	  GST_DEBUG (" wenfeng caps name %s", na);
	  
	  if(strstr(na, "audio") !=NULL)
	  {
		  GST_DEBUG (" wenfeng audio return");
		  return FALSE;
	  }
...
// vvv wenfeng
  ...
  gchar *na = gst_structure_get_name(structure11);
    ...
    if(strstr(na, "audio") !=NULL)
	  {
		  GST_DEBUG (" wenfeng rtsp server not support audio");
		 goto no_factory;
	  }
...
// vvv wenfeng
  ...
  char *pp = gst_plugin_feature_get_name (GST_PLUGIN_FEATURE (factory));
...
else
{
  ...
  GstPad *sinkpad1, *srcpad1;
   ...
   srcpad1 = gst_element_get_static_pad (queue, "src");

  gst_pad_link (pad, sinkpad1);  // wenfeng
  
  sinkpad = gst_element_get_static_pad (payloader, "sink");
//  gst_pad_link (pad, sinkpad);
   gst_pad_link (srcpad1, sinkpad);
  gst_object_unref (sinkpad);
  gst_object_unref (pad);
...
element22= gst_bin_get_by_name (GST_BIN(uribin),"source");
 ...
 g_object_set (element22, "keep-alive-string","wenfeng",NULL);
  ...
  GST_DEBUG ("no-more-pads  udpsrc = %p", element22);
	...
	//GstIterator *iterator = gst_bin_iterate_elements(GST_BIN(uribin));
